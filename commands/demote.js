...
function buildDemoteMenu(targetId, currentRank, config) {
  ...
  return new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      .setCustomId(`demote_select:${targetId}`)
...
module.exports = {
  data: new SlashCommandBuilder()
    .setName('demote')
    .setDescription('Degradeer een lid')
    .addUserOption(option =>
      option
        .setName('gebruiker')
        .setDescription('De gebruiker die je wilt degraderen')
        .setRequired(true)
    ),
  ...
  async execute(interaction, { config, createBaseEmbed }) {
      ...
      await interaction.reply({
        embeds: [
          createBaseEmbed({
            ...
            description: 'Bots kunnen niet gedegradeerd worden.'
          })
        ],
        ephemeral: true
      });
    ...
    if (!currentRank) {
      await interaction.reply({
        embeds: [
          createBaseEmbed({
            title: 'Geen rang gevonden',
            description: 'Deze gebruiker heeft geen rang uit de demote-lijst.'
          })
        ],
        ephemeral: true
      });
    ...
    const menu = buildDemoteMenu(member.id, currentRank, config);

    if (!menu) {
      await interaction.reply({
        embeds: [
          createBaseEmbed({
            title: 'Geen lagere rang beschikbaar',
            description: `${member} heeft al de laagste promotierang.`,
  ...
  async handleComponent(interaction, { config }) {
    ...
    if (!interaction.customId.startsWith('demote_select:')) return false;
    ...
    if (!currentRank) {
      await interaction.reply({
        content: 'De gebruiker heeft geen geldige rang.',
        ephemeral: true
      });
      return true;
    }
    ...
    if (newIndex >= oldIndex) {
      await interaction.reply({
        content: 'Je kunt alleen degraderen naar een lagere rang.',
        ephemeral: true
      });
      return true;
    }
    ...
    const modal = new ModalBuilder()
      .setCustomId(`demote_modal:${targetMember.id}:${selectedRoleId}`)
      .setTitle('Reden van demotie');

    const reasonInput = new TextInputBuilder()
      .setCustomId('reason')
      .setLabel('Waarom wordt deze persoon gedegradeerd?')
      .setStyle(TextInputStyle.Paragraph)
      .setRequired(true)
      .setMinLength(3)
      .setMaxLength(500)
      .setPlaceholder('Bijvoorbeeld: inactiviteit, gedrag of interne beslissing');
  ...
  async handleModal(interaction, {
    ...
    if (!interaction.customId.startsWith('demote_modal:')) return false;
    ...
    try {
      if (rolesToRemove.length > 0) {
        await targetMember.roles.remove(
          rolesToRemove,
          `Demotie door ${interaction.user.tag}: ${reason}`
        );
      }
      ...
      const successEmbed = createBaseEmbed({
        title: 'Demotie uitgevoerd',
        description: `${targetMember} is succesvol gedegradeerd.`,
        ...
        fields: [
          ...
          { name: 'Reden van demotie', value: reason.slice(0, 1024), inline: false },
      ...
      if (config.PROMOTE_LOG_CHANNEL_ID) {
        const demoteChannel = await client.channels.fetch(config.PROMOTE_LOG_CHANNEL_ID).catch(() => null);

        if (demoteChannel && demoteChannel.isTextBased()) {
          const demotionTimestamp = Math.floor(Date.now() / 1000);
          ...
          const logEmbed = createBaseEmbed({
            ...
            fields: [
              ...
              { name: 'Oude Rang', value: currentRank.label, inline: true },
              { name: 'Nieuwe Rang', value: newRank.label, inline: true },
              { name: 'Datum en tijd van Demotie', value: `<t:${demotionTimestamp}:F>`, inline: false },
              { name: 'Reden van demotie', value: reason.slice(0, 1024), inline: false },
              { name: 'Wie de demotie heeft uitgevoerd', value: `${interaction.user}`, inline: false }
            ]
          });

          await demoteChannel.send({ embeds: [logEmbed] });
        } else {
          await sendBotLog(
            '✦ 𝑫𝒆𝒎𝒐𝒕𝒆 𝒌𝒂𝒏𝒂𝒂𝒍 𝒇𝒐𝒖𝒕',
            'Het demote log kanaal kon niet worden gevonden of is niet tekstgebaseerd.'
          );
        }
      }
      ...
      await queueMemberListUpdate(
        guild,
        `Demotie uitgevoerd voor ${targetMember.user.tag}`,
        3000
      );
      ...
      if (interaction.replied || interaction.deferred) {
        await interaction.followUp({
          content: `Er ging iets mis bij het uitvoeren van de demotie: ${error.message}`,
          ephemeral: true
        }).catch(() => null);
      } else {
        await interaction.reply({
      ...
      await sendBotLog(
        '✦ 𝑫𝒆𝒎𝒐𝒕𝒊𝒆 𝒇𝒐𝒖𝒕',
        `Demotie uitvoeren voor **${targetMember.user.tag}** is mislukt.`,
        [
